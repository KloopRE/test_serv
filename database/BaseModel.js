...
export default class BaseModel {
    ...
    async save(filename, callback=()=>{}) {
        await fs.readFile(
            path.resolve(__dirname, 'models', filename),
            async (error,
                   buffer
            ) => {
    ...
    static find(query) {
        return JSON.parse(
            fs.readFileSync(path.resolve(__dirname, 'models', this.filename))
        ).filter(
    ...
    static async deleteOne(query, callback=()=>{}) {
        ...
        await fs.writeFile(
            path.resolve(__dirname, 'models', this.filename),
            JSON.stringify(data),
            async (error) => {callback()}
        )
    }
